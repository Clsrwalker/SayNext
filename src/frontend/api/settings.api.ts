...
export const pauseForReading = async (userId: string): Promise<MergeSettings> => {
  return updateUserSettings(userId, { pausedForReading: true } as Partial<Omit<MergeSettings, 'userId'>>);
};

export const resumeAutomatic = async (userId: string): Promise<MergeSettings> => {
  return updateUserSettings(userId, { pausedForReading: false } as Partial<Omit<MergeSettings, 'userId'>>);
};

export const displayInsightForReading = async (userId: string, displayText: string): Promise<MergeSettings> => {
  const apiUrl = getApiUrl();
  const response = await fetch(`${apiUrl}/api/settings`, {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ userId, control: 'display', displayText }),
  });

  if (!response.ok) {
    throw new Error('Failed to display insight for reading');
  }
