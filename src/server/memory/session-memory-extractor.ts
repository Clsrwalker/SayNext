...
function validateCandidate(input: {
  ...
  if (input.candidate.candidateType === "knowledge_fact") {
    flags.push("generic_knowledge_is_not_personal_memory");
  }
  ...
  const hardRejectFlags = new Set([
    ...
    "generic_knowledge_is_not_personal_memory",
  ]);
  ...
  return {
    ...
    reason: valid
      ? safeToPromote
        ? "Grounded candidate approved for promotion."
        : "Grounded candidate, but keep pending until Xiang explicitly reviews it."
...
- Personal memory is only detailed, Xiang-specific information that a general LLM could not know.
- Generic technical facts, interview advice, job descriptions, answer templates, and model-generated conclusions are not memory. Do not return them as candidates; put them in rejected with reason "generic knowledge is not personal memory" when useful for audit.
- If something is a public lecture, public video, news, movie, or third-party conversation, do not turn it into Xiang personal memory.
...
- knowledge_fact: legacy type only; do not return it because general knowledge is not personal memory.
...
- preserve concrete details such as situation, Xiang's action or decision, technologies or constraints, and result or limitation when available
- specific enough to distinguish this real experience from a generic LLM answer
...
- generic facts an LLM can answer without knowing Xiang
- suggested answers, interview strategy, or role-to-project matching analysis
...
- content should be one detailed factual paragraph, not a full transcript and not an answer script.
