...
touch .env
tmp=".env.tmp"
grep -v -E '^(SAYNEXT_RUNTIME_MODE|OPENAI_CONVERSATION_STATE_ENABLED|OPENAI_CONVERSATION_KEEP_ASSISTANT_OUTPUTS|OPENAI_CONVERSATION_WARMUP_TIMEOUT_MS|SESSION_MEMORY_PROVIDER|SESSION_MEMORY_BATCH_ENABLED)=' .env > "`$tmp" || true
mv "`$tmp" .env
cat >> .env <<'EOF'
SAYNEXT_RUNTIME_MODE=local
OPENAI_CONVERSATION_STATE_ENABLED=false
